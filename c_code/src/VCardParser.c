...
VCardErrorCode validateCard(const Card* obj) {
    ...
    int listLen = getLength(optionalProperties);
...
char* printProperty(void* toBePrinted) {
    ...
    int totalLength = nameLen + groupLen + parameterStringLen + valuesStringLen + 9;
...
char* printParameter(void* toBePrinted) {
    ...
    if ((nameLen == 0)) {
        char * placeholderString = malloc(sizeof(char) * 2);
        strcpy (placeholderString, "");
        return placeholderString;
    }

    int totalLength = nameLen + valueLen + 4;
    char * finalString = malloc(sizeof(char) * totalLength);
...
char* printValue(void* toBePrinted) {
    ...
    char * finalString = malloc(sizeof(char) * length+2);
...
char* printDate(void* toBePrinted) {
    ...
    if(value == true) {
        char * result = malloc(sizeof(char) * strlen(temp->text)+2);
...
char* printCard(const Card* obj) {
    ...
    int totalLen = propLen + addPropLen + bdayLen + anniLen + 5;
...
char* strListToJSON(const List* strList) {
    if (strList == NULL) {
        char * emptyString = malloc(sizeof(char) * 3);
        strcpy(emptyString, "[]");
        return emptyString;
    }
